...
import TaskRunnerDisplay from '../components/TaskRunnerDisplay';
...
const Map2DPage: React.FC<Map2DPageProps> = ({
  ...
  // Control collapse buttons - separate states
  ...
  const saveWaypointToBackend = async (waypointData: any) => {
    try {
      const response = await fetch(getApiUrl('/api/waypoints'), {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          x: waypointData.x,
          y: waypointData.y,
          theta: waypointData.theta,
          description: `Saved from current robot position at ${new Date().toLocaleString()}`
        })
      });

      if (response.ok) {
        const result = await response.json();
        logInfo(`Waypoint saved to backend: ${result.name}`, 'Map2D');
      } else {
        throw new Error('Failed to save waypoint to backend');
      }
    } catch (error) {
      logError('Error saving waypoint to backend:', 'Map2D', error);
      setSnackbar({
        open: true,
        message: `Failed to save waypoint: ${error.message}`,
        severity: 'error'
      });
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3} >
            ...
            <Paper ref={mainRef} sx={{ p: 2 }}>
              ...
              >
                ...
                >
                  ...
                  <IconButton
                    size="small"
                    onClick={() => setRobotControlCollapsed(prev => !prev)}
                  >
                    {robotControlCollapsed ? <ExpandMoreIcon /> : <ExpandLessIcon />}
                  </IconButton>
                </Box>

                {/* Visual WASD Layout - Non-interactive */}
                <Collapse in={!robotControlCollapsed}>
              ...
              {/* Embedded Map Control - Bottom Left Corner */}
              <Paper
                sx={{
                  position: 'fixed',
                  bottom: 16,
              ...
              >
                ...
                <Box
                  ...
                  onClick={() => setMapControlCollapsed(!mapControlCollapsed)}
                >
                  <Typography variant="subtitle2" sx={{ fontWeight: 'bold', m: 0 }}>
                    {t('map2d.controls.title', '🎛️ Map Controls')}
                  </Typography>
                  <IconButton sx={{ color: 'black' }}>
                    {mapControlCollapsed ? <ExpandMoreIcon /> : <ExpandLessIcon />}
                  </IconButton>
                </Box>

                {/* Map Controls Content */}
                <Collapse in={!mapControlCollapsed}>
                  ...
                  <Box sx={{ p: 1, maxHeight: '50vh', overflow: 'auto' }}>
                    ...
                    <Card sx={{ mb: 2, opacity: positionMode === 'send_to_ros' ? 0.6 : 1, fontSize: '0.8rem'}}>
                      <CardContent>
                        <Typography variant="subtitle1" gutterBottom>
                      ...
                      <CardContent sx={{ p: 2, '&:last-child': { pb: 1 } }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                          <FormControlLabel
                            ...
                            label={<Typography variant="caption">{t('map2d.show_lidar', 'Show LiDAR Scan')}</Typography>}
                          />
                          <FormControlLabel
                            control={
                              <Switch
                            ...
                            label={<Typography variant="caption">{t('map2d.show_path', 'Show Path')}</Typography>}
                          />
                          <FormControlLabel
                            control={
                              <Switch
                            ...
                            label={<Typography variant="caption">{t('map2d.show_goals', 'Show Goals')}</Typography>}
                          />
                          <FormControlLabel
                            control={
                              <Switch
                            ...
                            label={<Typography variant="caption">{t('map2d.auto_center', 'Auto Center on Robot')}</Typography>}
                          />
                        </Box>
                      </CardContent>
                    </Card>
        ...
        <Grid item xs={12} md={4} sx={{height:'100%'}}>
            {/* Task Runner Display */}
            <Grid item sx={{ overflow: 'auto' , height: mainHeight }}>
              <TaskRunnerDisplay
                currentTask={currentTask}
                onSaveCurrentPosition={() => {
                  const pos = robotData.pose;
                  if (pos) {
                    // Save point logic here - could save to localStorage or send to backend
                    const savedPoint = {
                      id: `point_${Date.now()}`,
                      name: `Điểm ${new Date().toLocaleTimeString()}`,
                      x: pos.x,
                      y: pos.y,
                      theta: pos.theta,
                    ...
                    // Save to backend API
                    saveWaypointToBackend(savedPoint);

                    setSnackbar({
                      open: true,
                      message: `Đã lưu điểm: (${pos.x.toFixed(2)}, ${pos.y.toFixed(2)})`,
                      severity: 'success'
                    });
                  }
                }}
                robotPosition={robotData.pose}
              />
          </Grid>
        </Grid>
