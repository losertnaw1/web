...
const EditMapPage: React.FC<EditMapPageProps> = ({ mapId, onNavigate }) => {
  
  // Map state
  const [originalMap, setOriginalMap] = useState<SavedMap | null>(null);
  ...
  useEffect(() => {
    const loadMap = async () => {
      ...
      try {
        ...
        const mapData: SavedMap = await response.json();
        
        setOriginalMap(mapData);
        setElements(mapData.elements);
        setMapName(mapData.name);
        setMapWidth(mapData.width);
        setMapHeight(mapData.height);
        setMapResolution(mapData.resolution);
        setHistory([mapData.elements]);
        setHistoryIndex(0);
  ...
  const resetToOriginal = () => {
    if (originalMap) {
      setElements(originalMap.elements);
      setMapName(originalMap.name);
      setMapWidth(originalMap.width);
      setMapHeight(originalMap.height);
      setMapResolution(originalMap.resolution);
      setHistory([originalMap.elements]);
      setHistoryIndex(0);
    }
  };
  ...
  const saveMap = async () => {
    ...
    if (!originalMap || !mapId) {
      setError('Original map data not found');
      return;
    }
    ...
    try {
      ...
      setSuccess(`Map "${mapName}" updated successfully!`);
      
      // Update original map reference
      setOriginalMap(mapData);
  ...
  // Check if map has changes
  const hasChanges = () => {
    if (!originalMap) return false;
    
    return (
      mapName !== originalMap.name ||
      mapWidth !== originalMap.width ||
      mapHeight !== originalMap.height ||
      mapResolution !== originalMap.resolution ||
      JSON.stringify(elements) !== JSON.stringify(originalMap.elements)
    );
  };
  ...
  if (!originalMap) {
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Button onClick={() => onNavigate?.('map-editor')} sx={{ mt: 2 }}>
          Back to Maps
        </Button>
      </Box>
    );
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>
        ✏️ Edit Map: {originalMap.name}
      </Typography>
      <Typography variant="subtitle1" color="text.secondary" gutterBottom>
        Modify and update existing map
      </Typography>
          ...
          <Paper sx={{ p: 2, mb: 2 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <Button
                ...
                onClick={saveMap}
                size="small"
                disabled={!mapName.trim() || loading || !hasChanges()}
                color="primary"
              >
                {loading ? 'Saving...' : 'Update Map'}
              </Button>
          ...
          <Paper sx={{ p: 2 }}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Typography variant="caption" component="div">
                ID: {originalMap.id}<br/>
                Created: {new Date(originalMap.created).toLocaleDateString()}<br/>
                Modified: {new Date(originalMap.modified).toLocaleDateString()}
              </Typography>
            </Box>
